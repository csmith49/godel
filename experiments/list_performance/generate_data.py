...
cmds = [(k, v.format(godel=GODEL, bm=args.benchmark, rules=RULESTRING)) for k, v in [
    ...
    ("BUN", "{godel} -target {bm} -stats -no-kbo {rules}"),
    ("BUNK", "{godel} -target {bm} -stats {rules}"),
    ("TD", "{godel} -target {bm} -stats -strategy td"),
    ("TDN", "{godel} -target {bm} -stats -strategy td -no-kbo {rules}"),
    ("TDNK", "{godel} -target {bm} -stats -strategy td {rules}")
]]
